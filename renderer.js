...
function renderFPSession(session) {
  ...
  if (fpData.completed) {
    programsEl.innerHTML = `
      <div style="color:#4caf50; font-size:14px; padding:12px;
        background:#0a1a0a; border:1px solid #2d5a2d; border-radius:10px;">
        ✓ Sesión ${session} completada.
      </div>`;
  ...
  programsEl.innerHTML = `
    <div style="margin-bottom:16px;">
      ${drivers.map((driver) => `
        <div style="margin-bottom:20px;">
          <div style="color:#aaa; font-size:13px; margin-bottom:10px; font-weight:700;">
            #${driver.number} ${driver.name}
          </div>
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;"
            id="${fpKey}Programs_${driver.id}">
            ${FP_PROGRAMS.map((prog) => `
              <div onclick="selectFPProgram('${fpKey}', ${driver.id}, '${prog.id}')"
                id="${fpKey}Prog_${driver.id}_${prog.id}"
                style="
                  background:#161616; border:1px solid #2a2a2a;
                  border-radius:10px; padding:12px; cursor:pointer;
                  transition:0.2s;
                ">
                <div style="font-size:18px; margin-bottom:6px;">${prog.icon}</div>
                <div style="color:#fff; font-size:13px; font-weight:700;
                  margin-bottom:4px;">${prog.name}</div>
                <div style="color:#666; font-size:11px;">${prog.description}</div>
              </div>
            `).join("")}
          </div>
        </div>
  ...
  actionsEl.innerHTML = `
    <button class="btn" style="width:240px;"
      id="${fpKey}RunBtn" onclick="runFPSession('${session}')" disabled>
      CORRER ${session}
    </button>
  `;
}
...
function ltUpdateInfoBar(state) {
  ...
  if (lapEl) {
    if (state.session === "qualifying") {
      ...
      let phaseStart = 0;
      let phaseDuration = 1080; // Q1 18min
      ...
      lapEl.textContent = leader ? leader.currentLap : "0";
    }
  }

  const weatherEl = document.getElementById("ltWeather");
  if (weatherEl) {
    // Solo mostrar clima si la sesión es carrera
    if (state.session === "race") {
      weatherEl.textContent = weatherLabel(state.weather);
    } else {
      weatherEl.textContent = "Seco";
    }
  }
}
...
function ltOpenPitwall() {
  ...
  modal.innerHTML = `
    <div style="background:#111; border:1px solid #333; border-radius:16px;
      padding:28px; max-width:700px; width:95%; margin:20px auto;">

      <div style="display:flex; justify-content:space-between; align-items:center;
  margin-bottom:6px;">
  <div style="color:#ffcc00; font-size:12px; font-weight:700;
    letter-spacing:1px;">🎙️ PITWALL</div>
  ${LTE.state.session === "practice" ? `
    <button onclick="ltClosePitwall(false); openSetupMenu();"
      id="btnSetupAlert"
      style="
        background:${getUnreadFeedbackCount() > 0 ? '#1a0800' : '#111'};
        border:1px solid ${getUnreadFeedbackCount() > 0 ? '#ffcc00' : '#333'};
      ...
      ${getUnreadFeedbackCount() > 0 ? `
        ...
        ">${getUnreadFeedbackCount()}</span>
      ` : ""}
    </button>
  ` : ""}
</div>
      <div style="font-size:18px; font-weight:800; color:#fff; margin-bottom:20px;">
        Gestión de pilotos — ${LTE.state.race?.name || ""}
      </div>
      ...
      ${playerDrivers.map((d) => `
        <div style="background:#0a0a0a; border:1px solid #222; border-radius:12px;
          ...
          <div style="display:flex; justify-content:space-between;
            ...
            <div>
              <div style="font-size:16px; font-weight:800; color:#4caf50;">
                #${d.driverNumber} ${d.driverName}
              </div>
              <div style="color:#aaa; font-size:12px; margin-top:3px;">
                P${d.position} · ${d.inPit ? "EN BOXES" : "EN PISTA"} ·
                Vuelta ${d.currentLap}
                ${isRace ? `· 🔋 ${Math.round(d.battery)}% · ⛽ ${Math.round(d.fuel)}%` : ""}
              </div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:13px; color:#aaa;">Compuesto actual</div>
              <div style="font-size:15px; font-weight:700; color:#fff;">
                ${compoundLabels[d.compound] || d.compound}
                (${d.lapsOnTyre} vts.)
              </div>
            </div>
          </div>
...
<div style="background:#111; border-radius:8px; padding:12px;">
  <div style="color:#aaa; font-size:11px; font-weight:700; margin-bottom:8px;">
    ${LTE.state.session === "practice" ? "PROGRAMA ACTUAL" : "BOXES"}
  </div>
  ...
  ${LTE.state.session === "practice" ? (() => {
    ...
    return `
      <div style="background:#0a0a0a; border:1px solid #222; border-radius:8px;
        ...
        <div style="color:#ffcc00; font-size:11px; margin-bottom:4px;">
          ${idx + 1} / ${total} — ${prog.icon} ${prog.name}
        </div>
        <div style="color:#aaa; font-size:12px;">
          ${nextPlan.laps} vueltas ·
          ${compounds.find((c) => c.id === nextPlan.compound)?.name || nextPlan.compound}
        </div>
        <div style="background:#1a1a1a; border-radius:20px; height:4px;
          overflow:hidden; margin-top:8px;">
          <div style="width:${Math.min(100, ((d._practiceLapsDone || 0) / nextPlan.laps) * 100)}%;
            height:100%; background:#e10600; border-radius:20px;"></div>
        </div>
        <div style="color:#666; font-size:11px; margin-top:4px;">
          ${d._practiceLapsDone || 0} / ${nextPlan.laps} vueltas completadas
        </div>
      </div>

      ${d.inPit ? `
        <button onclick="ltPitwallAction('${d.driverId}', 'send_out', '${nextPlan.compound}')"
          style="width:100%; background:#0a1a0a; border:1px solid #4caf50; color:#4caf50;
          border-radius:6px; padding:10px; cursor:pointer; font-size:12px; font-weight:700;">
          🟢 LANZAR A PISTA
        </button>
      ` : `
        <div style="color:#4caf50; font-size:12px; text-align:center; padding:8px;">
          ✓ En pista
        </div>
      `}
      ...
      ${(d._practicePrograms?.slice(idx + 1, idx + 3) || []).map((p, i) => {
        ...
        return `<div style="color:#444; font-size:11px; margin-top:6px;">
    ...
    <div style="display:grid; gap:6px;">
      ${circuitCompounds.map((c) => `
        <button onclick="ltPitwallAction('${d.driverId}', 'send_out', '${c}')"
          style="background:#0a0a0a; border:1px solid #333; color:#fff;
          border-radius:6px; padding:7px; cursor:pointer; font-size:11px;
          display:flex; align-items:center; gap:6px;">
          ${(() => {
            const cl = getCompoundLabel(c);
            return `<span style="background:${cl.bg}; color:${cl.color};
      ...
      `).join("")}
      <button onclick="ltPitwallAction('${d.driverId}', 'send_out', '${d.compound}')"
        style="background:#1a1a0a; border:1px solid #ffcc00; color:#ffcc00;
        border-radius:6px; padding:7px; cursor:pointer; font-size:11px;">
        🔄 Mismo compuesto (usado)
      </button>
    </div>
  ` : `
    <button onclick="ltPitwallAction('${d.driverId}', 'pit_in', null)"
      style="background:#1a0a0a; border:1px solid #e10600; color:#e10600;
      border-radius:6px; padding:10px; cursor:pointer; font-size:12px;
  ...
  `}
            ...
            <div style="background:#111; border-radius:8px; padding:12px;">
              <div style="color:#aaa; font-size:11px; font-weight:700;
                margin-bottom:8px;">MODOS</div>

              <div style="color:#aaa; font-size:10px; margin-bottom:4px;">ERS</div>
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:4px;
                margin-bottom:8px;">
                ${["recarga","neutral","boost","overtake","qualy"].map((mode) => `
                  <button onclick="ltPitwallAction('${d.driverId}', 'ers', '${mode}')"
                    style="background:${d.ersMode === mode ? "#1a1a00" : "#0a0a0a"};
                    border:1px solid ${d.ersMode === mode ? "#ffcc00" : "#222"};
                    color:${d.ersMode === mode ? "#ffcc00" : "#aaa"};
                    border-radius:5px; padding:5px; cursor:pointer; font-size:10px;
...
function ltPitwallAction(driverId, action, value) {
  ...
  d.compound        = value || nextPlan?.compound || d.compound;
  d.lapsOnTyre      = 0;
  d.lapPhase        = "outlap";
  d.lapPhaseCount   = 0;
  d.timeInSector    = 0;
      ...
      ">
        ...
        <td style="padding:6px 4px; text-align:center;">
          ${(() => {
            ...
            const bg    = isSoft ? "#e10600" : isMedium ? "#ffcc00" : isHard ? "#ffffff" : isInter ? "#4caf50" : isWet ? "#2196f3" : "#666";
            const color = isSoft ? "#fff" : "#000";
            const label = isSoft ? "B" : isMedium ? "M" : isHard ? "D" : isInter ? "I" : isWet ? "W" : "?";

            return `<span style="background:${bg}; color:${color}; font-size:10px;
              font-weight:800; padding:2px 5px; border-radius:3px;">${label}</span>`;
          })()}
        </td>
        <td style="padding:6px 4px; text-align:center; color:#666; font-size:12px;">${d.lapsOnTyre > 0 ? d.lapsOnTyre : "-"}</td>
...
function renderSetupSlider(param, ds) {
  ...
  let hintClass = "hint-ok";
  let hintText  = "OK";

  if (Math.abs(diff) <= range * 0.05) {
    hintClass = "hint-optimal";
    hintText  = "✓ ÓPTIMO";
  ...
  // Gradiente del slider según posición
...
function saveSetupChanges() {
  ...
  // Feedback rápido sin programa
  const driver = drivers.find((d) => d.id === driverId);
  if (improved) {
    ds.pendingFeedback.push({
      id:          Date.now(),
      programName: "Ajuste de reglajes",
      text:        `${driver?.name}: Cambio de setup confirmado. Score mejoró de ${prevScore}% a ${newScore}%. ${optimal ? "¡Setup en punto óptimo!" : "Seguimos afinando."}`,
      score:       newScore,
  ...
  const saveBtn = document.querySelector("[onclick='saveSetupChanges()']");
  if (saveBtn) {
