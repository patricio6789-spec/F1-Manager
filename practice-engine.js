...
const SETUP_PARAMS = [
  { id: "frontWing",      label: "Alerón delantero",      min: 1,   max: 50,  step: 1,   unit: "",  affects: "Grip delantero · subviraje" },
  { id: "rearWing",       label: "Alerón trasero",         min: 1,   max: 50,  step: 1,   unit: "",  affects: "Grip trasero · velocidad punta" },
  { id: "diffAccel",      label: "Diferencial aceleración",min: 50,  max: 100, step: 1,   unit: "%", affects: "Tracción · subviraje salida curva" },
  { id: "diffBrake",      label: "Diferencial frenada",    min: 50,  max: 100, step: 1,   unit: "%", affects: "Estabilidad · sobreviraje entrada" },
  { id: "suspFront",      label: "Suspensión delantera",   min: 1,   max: 11,  step: 1,   unit: "",  affects: "Rigidez · respuesta" },
  { id: "suspRear",       label: "Suspensión trasera",     min: 1,   max: 11,  step: 1,   unit: "",  affects: "Tracción · estabilidad" },
  { id: "arbFront",       label: "ARB delantero",          min: 1,   max: 11,  step: 1,   unit: "",  affects: "Balance aerodinámico" },
  { id: "arbRear",        label: "ARB trasero",            min: 1,   max: 11,  step: 1,   unit: "",  affects: "Estabilidad en curva" },
  { id: "brakePressure",  label: "Presión de frenos",      min: 50,  max: 100, step: 1,   unit: "%", affects: "Distancia frenado" },
  { id: "brakeBias",      label: "Balance de frenos",      min: 50,  max: 70,  step: 1,   unit: "%", affects: "Estabilidad bajo frenada" },
  { id: "rideHeight",     label: "Altura del coche",       min: 1,   max: 11,  step: 1,   unit: "",  affects: "Downforce · riesgo DSQ" },
];
...
function generateDriverOptimalSetup(driver, circuit) {
  // Cada piloto tiene valores óptimos únicos generados según sus atributos
  ...
  // Circuito influye en los óptimos
  const isDeg       = circuit?.degradation === "high";
  const isStreet    = circuit?.type === "street";
  const isHighSpeed = circuit?.tyreStress > 0.8;

  // Fórmula: valor base del rango + modificador según piloto/circuito
  const opt = (min, max, factor) =>
    Math.round(min + (max - min) * Math.max(0, Math.min(1, factor)));

  return {
    frontWing:     opt(1,  50,  isStreet ? 0.75 : isDeg ? 0.6 : 0.5 + (pace - 75) / 100),
    rearWing:      opt(1,  50,  isStreet ? 0.7  : isHighSpeed ? 0.3 : 0.5),
    diffAccel:     opt(50, 100, 0.3 + (aggression - 70) / 100),
    diffBrake:     opt(50, 100, 0.4 + (tyre - 70) / 200),
    suspFront:     opt(1,  11,  isStreet ? 0.3 : 0.5 + (pace - 75) / 200),
    suspRear:      opt(1,  11,  isDeg ? 0.6 : 0.5),
    arbFront:      opt(1,  11,  isHighSpeed ? 0.6 : 0.45),
    arbRear:       opt(1,  11,  isDeg ? 0.55 : 0.5),
    brakePressure: opt(50, 100, 0.5 + (aggression - 70) / 100),
    brakeBias:     opt(50, 70,  0.5 + (techFB - 70) / 200),
    rideHeight:    opt(1,  11,  isStreet ? 0.6 : isDeg ? 0.4 : 0.35),
  };
}
...
function calculateSetupScore(currentSetup, optimalSetup) {
  // Devuelve 0–100 según qué tan cerca está el setup actual del óptimo
  let totalScore = 0;
  let count = 0;
  ...
  SETUP_PARAMS.forEach((param) => {
    ...
    const score    = Math.max(0, 1 - distance * 2); // 0 = muy lejos, 1 = óptimo
    totalScore += score;
    count++;
  });
...
function initPracticeSessionState(sessionKey, circuit, race, weather, allDrivers, teams) {
  ...
  allDrivers.filter((d) => d.role !== "tester").forEach((driver) => {
    ...
    let aiPrograms = [];
    if (!isPlayer) {
      aiPrograms = generateAIProgramPlan(sessionKey, driver, team, weather);
    }
...
function generateAIProgramPlan(sessionKey, driver, team, weather) {
  const config    = SESSION_CONFIG[sessionKey];
  const maxLaps   = config.lapsAvailable.min + Math.floor(Math.random() * (config.lapsAvailable.max - config.lapsAvailable.min + 1));
  ...
  if (sessionKey === "FP1" && Math.random() < 0.7) {
    ...
    plan.push({ programId: "install", laps, compound: "medium" });
    totalLaps += laps;
  }
  ...
  for (const progId of extras.sort(() => Math.random() - 0.5)) {
    ...
    const laps = prog.lapsMin + Math.floor(Math.random() * (prog.lapsMax - prog.lapsMin + 1));
    if (totalLaps + laps > maxLaps) continue;
    plan.push({ programId: progId, laps, compound: prog.tyreRecommended });
    totalLaps += laps;
  }
