...
const LTE = {
  ...
  init(sessionType, driversData, teamsData, circuit, race, weather, weatherTimeline, compounds, practiceData) {
    ...
    const sessionDuration = isRace ? laps * (baseT + 5) : 2700;
    ...
    this.drivers = driversData
      ...
      .map((driver) => {
        ...
        const lapTimeBase = baseT - (combinedScore / 100) * 14.0 - rubberBonus;
...
if (this.state.session === "practice" && d._practiceLapsDone !== undefined) {
  d._practiceLapsDone++;
  // Confianza sube por vuelta
  if (d.confidence < 100) {
  ...
  _tickRace() {
    ...
    this.drivers.forEach((d) => {
      ...
      const dnfChance   = (100 - combined) / (320 * 92);
  ...
  _completeSectorRace(d) {
    ...
    if (d.currentSector === 1) {
      ...
      // Actualizar ERS y combustible
      this._updateERSAndFuel(d, lapTime);

      // IA gestiona ERS y combustible
      if (!d.isPlayer) {
        this._aiManageERS(d);
      }

      // Registrar vuelta en inventario de neumáticos
      if (typeof recordTyreLap === "function") {
        recordTyreLap(d.driverId);
      }

      if (lapTime < d.bestLap) {
        d.bestLap = lapTime;
      ...
      // Pit stop — programado por estrategia O llamado por el jugador
  ...
  _getSectorTarget(d, factor, degradationPen) {
    ...
    // Efecto ERS y combustible por sector (dividido en 3)
    ...
    return (base + noise + deg + ersPerSector + fuelPerSector) * (factor || 1);
  },
    ...
    if (this.state.session === "qualifying") {
      ...
      sorted.forEach((d, i) => {
        ...
        if (leader && d !== leader) {
          ...
          d.gap = lapDiff > 0 ? `+${lapDiff} VTA` : d.totalRaceTime - leader.totalRaceTime;
        } else {
          d.gap = 0;
        }
